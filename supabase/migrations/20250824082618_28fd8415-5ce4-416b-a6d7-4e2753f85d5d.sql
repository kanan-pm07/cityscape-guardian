...
CREATE TABLE IF NOT EXISTS public.billboard_reports (
...
CREATE TABLE IF NOT EXISTS public.permitted_billboards (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Create storage bucket for billboard images if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM storage.buckets WHERE id = 'billboard-images') THEN
...
DROP TRIGGER IF EXISTS update_billboard_reports_updated_at ON public.billboard_reports;
